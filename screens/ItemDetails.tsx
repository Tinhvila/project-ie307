...
export default function ItemDetails({ route }: { route: ItemDetailsScreenRouteProp }) {
  const { t } = useTranslation();
  const { image, title, description, rating, initialPrice, discountPrice, onAddToCart } = route.params;
  ...
  const startCountItemInterval = (type: 'increase' | 'decrease') => {
    ...
    intervalRef.current = setInterval(() => {
      if (type === 'increase')
        ...
        setCountItem(prev => (prev > 1 ? prev - 1 : prev));
    }, 100);
  }
  ...
  return (
    <ScrollView>
      <View>
        <Image src={image} alt={`Source: ${image}`} className={'bg-contain w-full h-80'} />
        <TouchableOpacity
      ...
      <View className={'m-5'}>
        <View className={'flex-row justify-between items-center'}>
          ...
          <View>
            <TouchableOpacity onPress={() => setFavorite(prev => !prev)}>
              <FontAwesomeIcon name={favorite ? 'heart' : 'heart-o'} size={24} />
            </TouchableOpacity>
          </View>
        </View>
        <View className={'flex flex-row items-center'}>
          ...
          {discountPrice && (<>
            <Text className={'ml-2 line-through text-gray-500'}>$ {initialPrice.toFixed(2)}</Text>
            <Text className={'ml-2 color-white bg-red-500 p-2 rounded-xl'}>{Math.round(100 - (discountPrice / initialPrice * 100)).toFixed(0)}%</Text>
          </>
          )}
        </View>
        <View className={'flex flex-row items-center justify-between my-5'}>
          <View className={'flex flex-row'}>
            ...
            <Text className={'ml-3 text-gray-700 text-lg'}>({rating ? rating.toFixed(1) : 0})</Text>
          </View>
          <View className={'flex-row items-center justify-center'}>
            <TouchableOpacity
              onPress={() => setCountItem((prev) => (prev === 1 ? prev : prev - 1))}
              onPressIn={() => startCountItemInterval('decrease')}
              onPressOut={stopCountItemInterval}
              disabled={countItem <= 1 ? true : false}
            >
              <FontAwesomeIcon name="minus" size={24} color={countItem === 1 ? 'gray' : 'black'} />
            </TouchableOpacity>
            <Text
              className={'mx-5 px-5 py-1 border-2 rounded-lg text-center'}
            >
              {countItem}
            </Text>
            <TouchableOpacity
        ...
        <TouchableOpacity
          className={'bg-black p-3 rounded-full items-center flex-row justify-center'}>
        ...
        <View>
          <Text className={'font-medium text-xl'}>{t("item_detail.description")}</Text>
          <Text className={'text-justify'}>{description}</Text>
        </View>
        <View className={'flex flex-row items-center justify-between bg-blue-200 p-3 my-3 rounded-full'}>
          ...
          <View className={'flex-row justifiy-center items-center'}>
            ...
            <Text className={'ml-1'}>{t("item_detail.delivery_option.free_delivery")}</Text>
          </View>
        </View>
      </View>
    </ScrollView>
